...
foreach $fqf (@fqs) {
	...
	if ($bar2==0){
		print "../scripts/trim2bRAD.pl $fqf \"$site\" \"$adaptor\" >$outname\n";
	}
	else {
		print "../scripts/trim2bRAD_2barcodes.pl fastq=$fqf site=\"$site\" barcode2=\"[ATGC]{$bar2}\" adaptor=\"$adaptor\" sampleID=$sampleid\n";
	}
}
